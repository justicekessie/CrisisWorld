...
function getFeedsFromEnv(): string[] {
...
async function run() {
  ...
  for (const feedUrl of feeds) {
    try {
      ...
      results.push(result);
      console.log(
        `Feed ${feedUrl}: fetched=${result.fetched}, inserted_sources=${result.insertedSources}, linked_incidents=${result.linkedIncidents}`
      );
    } catch (error) {
  ...
  console.log(
    `Ingestion finished. feeds=${results.length}, fetched=${totals.fetched}, inserted_sources=${totals.insertedSources}, linked_incidents=${totals.linkedIncidents}`
  );
}
